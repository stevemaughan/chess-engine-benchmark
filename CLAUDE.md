...
These instructions are addressed to the AI model reading this file. Read the
whole file before doing anything else, then begin.
...
to resume must be on disk (`docs/`, `source/`, `final/`). If you find
yourself starting with `docs/start_time.txt` already present, you are
**resuming, not starting** — see "Time-keeping" below.

## Identity

First establish which model you are: your model name and version, e.g.
"Fable 5.1", "Opus 5", "GPT-6", "Gemini 4". Use that name, exactly, for:

- Engine name (reported by `id name`): **`<model name> chess 24hrs`**
  (e.g. `Fable 5.1 chess 24hrs`)
- Author (reported by `id author`): **`<model name>`** (e.g. `Fable 5.1`)
- Executable name: **`<ModelName>chess24hrs.exe`** — the model name with
  spaces and dots removed, so it is easy to pass to tools
  (e.g. `Fable51chess24hrs.exe`)

Record the names you chose in `docs/progress.md` at the start. Wherever
`<ModelName>chess24hrs.exe` appears below, substitute your executable name.
...
- The target machine is a **modern Windows 11 laptop** (not this development
  ...
  part (which have slow microcoded PEXT). Do **not** assume AVX-512: the
  development machine may have it, but the target laptop may not. Compile
  for the `x86-64-v3` baseline (`-march=x86-64-v3`, optionally plus
  `-mtune=znver4` or similar), **never `-march=native`**.
...
4. **Language: C or C++** — your choice. The toolchain is GCC (MinGW-w64),
   installed and on the PATH as `gcc` / `g++`. Check the version with
   `gcc --version` at the start of the run. Do not assume MSVC or clang are
   available.
...
6. You may use up to **(physical cores − 2)** CPU cores for testing (fastchess
   `-concurrency`). Find the physical core count at the start of the run with:
   `powershell -NoProfile -Command "(Get-CimInstance Win32_Processor | Measure-Object -Property NumberOfCores -Sum).Sum"`
   and record the resulting `-concurrency` value in `docs/progress.md`. The
   fastchess examples below use `-concurrency 10` (a 12-core machine); adjust
   to your number.
...
```
resources\fastchess\fastchess.exe --compliance final\<ModelName>chess24hrs.exe
```
...
```
resources\fastchess\fastchess.exe ^
  -engine cmd=final\<ModelName>chess24hrs.exe name=mine ^
...
- **If `docs/start_time.txt` already exists, do NOT overwrite it.** It means
  ...
  `docs/progress.md`, your snapshots and the current state of `source/` and
...
- **What this is** — the benchmark: one model (you, named), 24 hours, from
...
## Keeping a way back (mandatory)

No version control is provided. You must still be able to return to a
known-good state when a change turns out to be a regression or introduces a
bug. Every time you reach a working, tested state, copy the source and the
build to a dated folder under `source/snapshots/` (e.g.
`source/snapshots/2026-08-22_1430_hour07/`) together with a short note saying
what changed and, if measured, its effect. Never overwrite or delete a
snapshot. If your tooling offers its own versioning you may use that instead,
provided the history stays on disk inside this project folder.
